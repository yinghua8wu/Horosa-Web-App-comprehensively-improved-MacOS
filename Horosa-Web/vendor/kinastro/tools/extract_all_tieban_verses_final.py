#!/usr/bin/env python3
"""
tools/extract_all_tieban_verses_final.py — 從《鐵板神數清刻足本.txt》提取 ALL 條文
...
def is_meaningful_verse(text):
    ...
    # 計算天乾地支比例 - 如果超過 50% 是天乾地支，過濾
    stem_branch_count = sum(1 for c in text_clean if c in HEAVENLY_STEMS | EARTHLY_BRANCHES)
    if stem_branch_count > len(text_clean) * 0.5:
        return False
    ...
    meaningful_words = [
        ...
        '一樹', '殘花', '枯枝', '復茂', '寄人', '廊廟', '如何', '自立',
        '消耗', '獨守', '若雨', '連宵', '日照', '紗窗', '紫艷明',
        '柳鶯', '枝土', '根新春', '亨通', '心安而', '意穩', '意外',
        '含哺', '笑', '輕借', '力便', '是運', '通時', '未滿', '六旬',
        '時亨', '運通', '鎮日', '春風', '殘花', '枯枝', '復茂',
    ]
...
def generate_json(verses, output_path):
    ...
    output_data = {
        "_meta": {
            ...
            "description": "鐵板神數條文資料庫 - 100% 來自原文（完整版，所有條文，已嚴格過濾密碼表）",
            "version": "13.0",
            "total_verses": len(verses),
            "categories": list(categories.keys()),
            "parser": "tools/extract_all_tieban_verses_final.py",
...
def main():
    """主函數"""
    print("=" * 70)
    print("鐵板神數原文條文提取工具（最終版 - 嚴格過濾密碼表）")
    print("=" * 70)
    print()
