#!/usr/bin/env python3
"""
tools/extract_tieban_verses_complete.py — 從《鐵板神數清刻足本.txt》提取所有條文
...
def parse_tieban_verses(file_path):
    ...
    for line_idx, line in enumerate(lines, 1):
        ...
        if match:
            ...
            # 嘗試匹配沒有數字的條文（如：亏亏灵灵 卜商之叹见于此年）
            match2 = re.match(r'^([^\s]+)\s+(.{4,})$', line)
            if match2:
                code = match2.group(1)
                text = match2.group(2).strip()
                
                if '卜商' in text or is_meaningful_verse(text):
                    verse_key = f"{verse_counter:04d}"
                    verses[verse_key] = {
                        'verse': text,
                        'category': '災厄',
...
def generate_json(verses, output_path):
    ...
    output_data = {
        "_meta": {
            ...
            "parser": "tools/extract_tieban_verses_complete.py",
