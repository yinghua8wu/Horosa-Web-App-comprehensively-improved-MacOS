...
export const BIFA_MATCHERS = {
	...
	39: (ctx)=>{ const vb = godBranch(ctx, '玄武'); const yj = zi(ctx.yueGeneralBranch); return (vb && yj && zi(vb) === yj) ? { evidence: ['玄武坐太阳月将之位（太阳照武宜擒贼；须昼占方验）'] } : null; },
	44: (ctx)=>{ return [ctx.firstGod, ctx.midGod, ctx.lastGod].every((g)=>g === '贵人') ? { evidence: ['三传天将皆乘贵人（遍地贵人，贵多反无依）'] } : null; },
	54: (ctx)=>{ if(!(ctx.courseName && ctx.courseName.indexOf('虎视') >= 0)) return null; const hasHu = [ctx.firstGod, ctx.midGod, ctx.lastGod].indexOf('白虎') >= 0 || godBranch(ctx, '白虎'); return hasHu ? { evidence: ['虎视课天将又乘白虎（虎视逢虎，至惊至危）'] } : null; },
	...
	67: (ctx)=>{ const HU = { '金': '肝经', '木': '脾经', '水': '心经', '火': '肺经', '土': '肾经' }; const vb = godBranch(ctx, '白虎'); if(!vb) return null; const down = ctx.upDownMap ? ctx.upDownMap[zi(vb)] : ''; const struck = (down && ke(down, vb)) || ke(ctx.dayGan, vb); if(!struck) return null; const m = HU[wxOf(vb)]; return m ? { evidence: [`白虎乘${zi(vb)}(${wxOf(vb)})受克：主${m}病（依五行论治）`] } : null; },
	...
	92: (ctx)=>{ const lb = godBranch(ctx, '青龙'); if(!lb) return null; const sheng = shenshaBranch(ctx, 'month', '生气'); return (gen(lb, ctx.dayGan) && sheng && zi(lb) === sheng && !inKong(ctx, lb)) ? { evidence: [`青龙乘${zi(lb)}生日干、值月内生气（龙加生气，吉迟迟）`] } : null; },
