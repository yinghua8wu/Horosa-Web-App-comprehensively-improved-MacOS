...
class BookList extends Component{
	...
	async requestBooks(){
		...
		const data = await request(`${ServerRoot}/astroreader/allbooks`, {
	...
	async requestRemoveBook(rec){
		...
		const data = await request(`${ServerRoot}/astroreader/removebook`, {
			body: JSON.stringify(params),
		});
		const result = data[ResultKey];
		...
		return (
			<div style={{height: tbly}}>
				...
				<Row gutter={12} style={{marginBottom: 10}}>
					...
					<Col offset={4} span={6}>
						<XQInput placeholder='所属用户' value={this.state.userId} onChange={this.changeUserId} />
					</Col>
