...
class HouseSu13 extends HouseSu{
	constructor(option){
		super(option)
	}
	...
	drawTitle(){
		let x = this.x + this.width - this.fontSize*2 - this.margin;
		let y = this.y + this.height - (this.fontSize + this.margin);
		let w = this.fontSize + this.margin * 2;
		let h = this.fontSize + this.margin;

		let data = [this.houseObj.name];

		this.titleSvg = drawTextH(this.svg, data, x, y, w, h, this.margin, Su28Helper.getSu28Color(13));		
		this.genTooltip();	
	}
	...
	drawStar(){
		...
		for(let i=0; i<startxts.length; i++){
			...
			let y = this.y + this.height - h - this.margin;
