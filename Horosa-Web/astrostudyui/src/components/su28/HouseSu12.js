...
class HouseSu12 extends HouseSu{
	constructor(option){
		super(option)
	}
	...
	drawTitle(){
		...
		let y = this.y + houseH - this.fontSize - this.margin;
		let w = this.fontSize + this.margin*2;
		let h = this.fontSize + this.margin;

		let data = [this.houseObj.name];
		this.titleSvg = drawTextH(this.svg, data, x, y, w, h, this.margin, Su28Helper.getSu28Color(12));	
		this.genTooltip();	
	}
