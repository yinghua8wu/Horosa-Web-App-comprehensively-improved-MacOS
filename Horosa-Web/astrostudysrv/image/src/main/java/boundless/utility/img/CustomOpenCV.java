...
public class CustomOpenCV {
	...
	public final static Logger logger = AppLoggers.getLog("opencv", "lib");
	...
	static enum OS {
		...
		public static OS getCurrent() {
			...
			for (final OS os : OS.values()) {
				if (os.is(osName)) {
					QueueLog.info(logger, "Current environment matches operating system descriptor \"{}\".", os);
					return os;
				}
			}
	...
	static enum Arch {
		X86_32("i386", "i686", "x86"), X86_64("amd64", "x86_64"), ARMv7("arm"), ARMv8("aarch64", "arm64");
		...
		public static Arch getCurrent() {
			...
			for (final Arch arch : Arch.values()) {
				if (arch.is(osArch)) {
					QueueLog.info(logger, "Current environment matches architecture descriptor \"{}\".", arch);
					return arch;
				}
			}
				...
				if (file.isDirectory() && file.getName().startsWith(OPENCV_PREFIX)) {
					try {
						...
						if (e.getCause() instanceof AccessDeniedException) {
							QueueLog.error(logger, "Failed delete a previous instance of the OpenCV binaries, "
	 ...
	 * {@link System#loadLibrary(String)} with {@link Core#NATIVE_LIBRARY_NAME}). If
	...
	private static class SharedLoader {
		// Class loader error messages indicating OpenCV is not found on
		// java.library.path
		...
		private SharedLoader() {
			try {
				...
				QueueLog.info(logger, "Loaded existing OpenCV library \"{}\" from library path.",
						Core.NATIVE_LIBRARY_NAME);
			} catch (final UnsatisfiedLinkError ule) {
				QueueLog.error(logger, ule);
				...
				if (ule == null || !openCVNotFoundInJavaLibraryPath(ule.getMessage())) {
					QueueLog.error(logger, "Encountered unexpected loading error.");
					throw ule;
				}
				...
				if (Double.parseDouble(System.getProperty("java.specification.version")) >= 12) {
					QueueLog.error(logger, "OpenCV.loadShared() is not supported in Java >= 12. Falling back to OpenCV.loadLocally().");
					loadLocally();
					return;
				}
				...
				QueueLog.info(logger, "OpenCV library \"{}\" loaded from extracted copy at \"{}\".", Core.NATIVE_LIBRARY_NAME, System.mapLibraryName(Core.NATIVE_LIBRARY_NAME));
			}
		}
		...
		private static void addLibraryPath(final Path path) {
			...
			try {
				...
				QueueLog.info(logger, "System library path now \"{}\".", System.getProperty("java.library.path"));
	...
	private static Path extractNativeBinary(final OS os, final Arch arch) {
		...
		case LINUX:
			...
			case X86_64:
				...
				break;
			case ARMv7:
				location = "/nu/pattern/opencv/linux/ARMv7/libopencv_java342.so";
				break;
			case ARMv8:
		...
		case OSX:
			...
			case X86_64:
				...
				break;
			case ARMv8:
				location = "/nu/pattern/opencv/osx/ARMv8/libopencv_java342.dylib";
				break;
			default:
				throw new UnsupportedPlatformException(os, arch);
			}
		...
		try {
			QueueLog.info(logger, "Copying native binary to \"{}\".", destination);
			Files.createDirectories(destination.getParent());
			Files.copy(binary, destination);
			binary.close();
		} catch (final IOException ioe) {
			QueueLog.error(logger, ioe);
			throw new IllegalStateException(String.format("Error writing native library to \"%s\".", destination), ioe);
		}

		QueueLog.info(logger, "Extracted native binary to \"{}\".", destination);
