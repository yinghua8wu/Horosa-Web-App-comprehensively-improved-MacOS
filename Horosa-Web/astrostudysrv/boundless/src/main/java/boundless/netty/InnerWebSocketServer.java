...
public final class InnerWebSocketServer extends WebSocketServer {
	
	private NettyUDPBroadcaster broadcaster;
	private ScheduledFuture broadSchedule;
	

	public InnerWebSocketServer(int port, Class decoder, Class encoder) {
		super(port, decoder, encoder);
	}

	public InnerWebSocketServer(int port, Class decoder) {
		super(port, decoder);
	}

	public InnerWebSocketServer(IServerConfiguration config, Class decoder, Class encoder) {
		super(config, decoder, encoder);
		initUdp();
	}

	public InnerWebSocketServer(IServerConfiguration config, Class decoder) {
		super(config, decoder);
		initUdp();
	}

	public InnerWebSocketServer(IServerConfiguration config) {
		super(config);
		initUdp();
	}
	
	public InnerWebSocketServer(int port, int udpPort, Class decoder){
		this(port, decoder);
