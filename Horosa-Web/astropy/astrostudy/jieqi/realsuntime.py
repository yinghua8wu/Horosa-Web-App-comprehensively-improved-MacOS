...
offsets = [
    ...
    -13 * 60 - 44, -13 * 60 - 38,
    # 2/24 原 -11*60-4 为转录笔误(邻值 -13:13 / -12:54,应 -13:04)
    -13 * 60 - 30, -13 * 60 - 22, -13 * 60 - 13, -13 * 60 - 4, -12 * 60 - 54, -12 * 60 - 43, -12 * 60 - 32,
    -12 * 60 - 21, -12 * 60 - 8,
    ...
    # 4/10-13 原为 +46/+30/+16/+1:`0*60+mm` 写法无法表达「-0:mm」,符号被吞;
    # 按邻值平滑(4/9=-1:01 → 4/14=+0:13,日差约 +15s)应为 -46/-30/-16/-1。
    -1 * 60 - 1, 0 * 60 - 46,
    ...
    2 * 60 + 22,
    # 12/25 原 +7 同上「负零时」符号被吞(邻值 +23 → -37,日差约 -30s,应 -0:07)
...
def getBaseLonByZone(zone):
    ...
    sym = zone[0:1]
    rest = zone[1:]
    # 解析「±HH:MM」的分钟段:原实现只取小时,印度 +05:30 被当 +05:00 → 真太阳时差半小时。
    hm = rest.split(':')
    try:
        h = int(hm[0])
    except (TypeError, ValueError):
