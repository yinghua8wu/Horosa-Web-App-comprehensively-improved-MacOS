...
## 六、改动文件清单（本次升级）
- 后端：`astropy/astrostudy/acg/ACGraph.py`（重写为解析法 + ls/parans/pointReport + 多点横线 + 曲线平滑闭合）、`astropy/websrv/webacgsrv.py`（`acgpoint`）、`astropy/astrostudy/acg/validate_acg.py`。
