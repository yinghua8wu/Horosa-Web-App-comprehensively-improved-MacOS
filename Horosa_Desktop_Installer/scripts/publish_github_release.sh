...
sections = [
    ...
    "- `v1.3.0` 汇总了 AI分析 技法挂载与自动快照补生成修复，并保留 ARM Mac 安装稳定性与可信快速启动策略。",
    "- `v1.3.0` rolls up the AIAnalysis technique-mounting and auto-snapshot fixes while keeping the ARM Mac install hardening and trusted fast-start path in place.",
