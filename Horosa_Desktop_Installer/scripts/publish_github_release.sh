...
sections = [
    ...
    "- `v1.2.4` 延续 `v1.2.3` 的 ARM Mac 安装稳定性修复，并加入“首次完整检查、后续可信快速启动”的策略，兼顾可靠性和日常启动速度。",
    "- `v1.2.4` keeps the ARM Mac installation hardening from `v1.2.3` and adds a trusted fast path after the first successful launch, improving daily startup time without giving up a full-check fallback.",
