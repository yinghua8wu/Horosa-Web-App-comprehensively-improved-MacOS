...
sections = [
    ...
    f"- `{tag_name}` 补齐主限法福点在 AI 导出中的覆盖，让 Web、App、表格结果与导出快照保持同一套结果口径。",
    f"- `{tag_name}` completes Part of Fortune primary-direction coverage in AI export, keeping Web, App, table output, and export snapshots on the same result path.",
