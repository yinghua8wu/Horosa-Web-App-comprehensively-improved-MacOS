...
read -r REPO_OWNER REPO_NAME TAG_PREFIX VERSION TAG_NAME RUNTIME_TAG_NAME RUNTIME_ASSET DESKTOP_ASSET DESKTOP_PKG DESKTOP_PKG_ZIP DESKTOP_OFFLINE_PKG DESKTOP_OFFLINE_PKG_ZIP UPDATE_MANIFEST_NAME RUNTIME_VERSION PRIMARY_DOWNLOAD SUPPORTED_ARCH RELEASE_CHANNEL <<EOF
...
sections = [
    ...
    f"- `{tag_name}` 是桌面成品 beta，统一桌面启动体验、README 展示、离线安装包、runtime 版本与 GitHub 发布清单。",
    f"- `{tag_name}` is the desktop-product beta aligning startup UX, README presentation, offline installer, runtime version, and GitHub release manifest.",
    "- 启动页重做为高信息密度 runtime / service / log / recovery / install-source 控制台，打开后不需要下滑即可看到关键状态。",
    "- The launcher is now a dense runtime / service / log / recovery / install-source console, with key status visible in the first viewport.",
    "- App 会记忆上次窗口大小，下次启动继续沿用。",
    "- The app remembers the previous window size and restores it on the next launch.",
    "- 一键打开 Web 与命令端路径移除了全新 Mac 上对系统 Python 的隐性假设。",
    "- The one-click Web and command paths no longer assume a system Python on clean Macs.",
    "- README 示例图替换为 2.0.0 的占星工作区、三式工作区与导航弹层三张截图。",
    "- README screenshots now show the 2.0.0 astrology workspace, Sanshi workspace, and navigation overlay.",
    "",
    "## 下载 / Download",
