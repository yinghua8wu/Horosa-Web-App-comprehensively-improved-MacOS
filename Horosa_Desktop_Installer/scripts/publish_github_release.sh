...
sections = [
    ...
    "- `v1.2.3` 重点加强 ARM Mac 安装稳定性：启动前先验证内置 Java，可疑时自动回退到用户 runtime，并允许从本地缓存 runtime 自恢复。",
    "- `v1.2.3` hardens ARM Mac installation stability by validating the embedded Java runtime up front, falling back to the user runtime when the shared runtime is incomplete, and recovering from cached local runtime archives.",
