...
- 星盘/盘面释义悬浮窗改成自有 body-portal 悬浮层，不再依赖页面容器的裁切与滚动环境；靠近窗口边缘时会自动回避，内容过长时在悬浮层内部滚动。
- 这次重点补的是“释义窗本身”的定位与挂载逻辑，避免旧版 `Popover` 在最外层固定布局和内部滚动容器并存时被窗口底边或容器边界截掉。
- 本次 release 提升到 `v1.0.26`，并同步发布新的独立 runtime `1.0.26-runtime1`，确保这次释义悬浮窗修复真正落到 release 下载件。
