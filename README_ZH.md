...
[入口页](README.md) | [英文说明](README_EN.md) | [v2.0.2 Beta](https://github.com/Horace-Maxwell/Horosa-Web-App-comprehensively-improved-MacOS/releases/tag/v2.0.2)

**当前版本：** `v2.0.2 beta`

**本次重点：** `v2.0.2 beta` 延续 2.0 桌面交付线，保留 2.0.1 的奇门遁甲一致性修复，并继续打磨全局桌面 UI。
...
`2.0.2 beta` 是 2.0 桌面发布线的聚焦修复版。它继续保留签名离线安装路径，保留 2.0.1 的奇门遁甲一致性修复，并继续收紧全局布局、明暗主题、导航入口与桌面交付。

这一版新增的关键能力包括：

- 奇门遁甲拆补与置润的起局结果对齐 Horosa 手机版 API
- 置润保留自己的节气段逻辑，但三元改按当前盘日柱判定
- 天盘干、地盘干、八门、九星、局数、值符值使已用 300 个拆补 + 300 个置润真实手机版 API 案例对照
- 本地兜底历法按准确交节时分判断节气，晚子时默认保持手机版兼容口径
- 桌面端八神显示有意保留既有 `虎 / 玄` 口径
- 全局侧栏、底部快捷功能、分至页、大六壬圆盘、八字运表与占星骰子盘面遮罩已按明暗主题继续打磨
- 清理重复导航入口，只保留更醒目的主入口
- `2.0.2 / 2.0.2-runtime1` 在 package、Tauri、release_config、README、manifest、app zip、离线 pkg 与 runtime 包中统一对齐
...
- 想看当前 beta 版本页面：看 [GitHub Release v2.0.2 Beta](https://github.com/Horace-Maxwell/Horosa-Web-App-comprehensively-improved-MacOS/releases/tag/v2.0.2)
- 想进入主工程：看 `Horosa-Web/`
