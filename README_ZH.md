...
**本次重点：** `v1.3.4` 修复遁甲天盘奇仪随时辰飞布的问题，确保遁甲单盘、三式合一与 AI 导出在 Web 和 App 中使用同一套正确结果。
...
- 想看当前版本页面：看 [GitHub Release v1.3.4](https://github.com/Horace-Maxwell/Horosa-Web-App-comprehensively-improved-MacOS/releases/tag/v1.3.4)
- 想进入主工程：看 `Horosa-Web/`
- 想看共享运行时与诊断：看 `runtime/` 与 `diagnostics/`

## 致谢

本项目为参考星阙 Horosa - 荀爽（Herakleios）所发布的星阙 App 和 Web，改良制作而成。

同时也郑重感谢郑大哥对原星阙体系的创建、长期维护与公开贡献。星阙能够被后来者继续研究、改良与分发，离不开这些早期工作的基础。请不要忘记爽哥和郑大哥的贡献。
