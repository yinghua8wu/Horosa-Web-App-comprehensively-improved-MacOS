...
[入口页](README.md) | [英文说明](README_EN.md) | [v2.0.0 Beta](https://github.com/Horace-Maxwell/Horosa-Web-App-comprehensively-improved-MacOS/releases/tag/v2.0.0)

**当前版本：** `v2.0.0 beta`

**本次重点：** `v2.0.0 beta` 是桌面成品 beta：重做启动页，记忆窗口大小，启动首屏不再需要下滑，替换 README 截图，并用公证离线安装包覆盖全新 Apple Silicon Mac 的首装路径。
...
`2.0.0 beta` 是这条发布线的桌面成品 beta。它不是单点功能补丁，而是把公开展示、启动体验、窗口行为、运行时、安装包、GitHub Release 和 manifest 一起对齐。

这一版新增的关键能力包括：

- 启动页重做为 runtime / service / log / recovery / install-source 控制台
- 启动首屏约束在第一视口内，打开后不需要下滑即可看到关键状态
- App 记忆上次窗口大小，下次启动自动沿用
- 一键打开 Web 与命令端路径移除全新 Mac 上对系统 Python 的隐性假设
- README 示例图替换为 2.0.0 的占星工作区、三式工作区与导航弹层
- `2.0.0 / 2.0.0-runtime1` 在 package、Tauri、release_config、README、manifest、app zip、离线 pkg 与 runtime 包中统一对齐
...
- 想看当前 beta 版本页面：看 [GitHub Release v2.0.0 Beta](https://github.com/Horace-Maxwell/Horosa-Web-App-comprehensively-improved-MacOS/releases/tag/v2.0.0)
- 想进入主工程：看 `Horosa-Web/`
